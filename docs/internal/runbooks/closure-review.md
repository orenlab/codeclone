...
| `unverified_python_outside_scope` | Python changed outside declared scope since intent start, claimed by no intent | Declare those paths in scope and verify with an after-run, or revert them |
| `own_unscoped_dirty` | Own unattributed changes of any type outside declared scope (`CODECLONE_STRICT_FINISH`) | Either remove out-of-scope changes (if unintended) or call `start_controlled_change` with expanded scope |
...
| Scope check reconciles intent snapshot against live tree; unattributed out-of-scope Python changed since intent start always blocks, other file types block only if CODECLONE_STRICT_FINISH is set | path_only | codeclone/surfaces/mcp/_workspace_intent_lifecycle.py |
