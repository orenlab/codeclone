...
| `CACHE_VERSION` | `"3.7"` | analysis cache format | Invalidates `.codeclone/cache.json` on mismatch. Bump on cache layout or serialization change. |
| `REPORT_SCHEMA_VERSION` | `"3.1"` | report artifact JSON | Governs report.json, report.sarif structure. Bump on schema shape change. |
