...
- `metrics` — object with `summary` and per-family `families` (complexity, coupling, cohesion, …). The `dead_code` family carries three lanes that are never summed: `items` (dead findings), `unresolved_overrides` (rule-3 abstentions on an external base) and `unresolved` (symbols with no internal evidence that a consumer outside the repository could reach - reported under the `open` world contract instead of being asserted dead; each row names its `reason`, `reachability`, `witness` and `world_contract`; the `witness` is `<kind>:<subject>`, the construct that puts the symbol on a public path — `public_module`, `package_reexport`, `declared_reexport` for a plain module whose `__all__` lists an import it re-exports, `star_reexport`, `exposed_subclass`, `exposed_ancestor` — or the construct that could not be read — `module_getattr`, `lazy_namespace`, `unresolved_base`; an `__all__` entry is exposure evidence, never internal use). `summary.unresolved` counts that lane and `summary.world_contract` names the world every verdict in the family was derived under
