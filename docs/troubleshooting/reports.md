...
3. **Cache** (`.codeclone/db/cache.sqlite3`) — incremental state to speed up re-runs
...
- **Check:** If findings differ between formats, the cache may be stale—delete `.codeclone/db/cache.sqlite3*` and re-run for a clean analysis.
