...
but response-budget enforcement is still observe-only. Finish responses keep
the human-readable receipt markdown inline and omit the duplicate nested typed
receipt; retrieve that complete structured receipt with
`get_review_receipt(root, run_id, receipt_digest, format="structured")`.
Patch Trail and blast artifact drill-down are durable, and memory/context
omitted tails have exact continuation routes, but default finish packing is not
enforced until an explicit omission policy is enabled.
