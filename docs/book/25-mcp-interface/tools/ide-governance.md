...
| `get_controller_audit_trail`  | `root`, `limit`, `audit_path` | Audit trail + payload footprint — same collector as CLI `--audit`. `limit` caps recent events (default 50). `audit_path` overrides the audit database location |
