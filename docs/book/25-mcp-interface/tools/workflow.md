...
`start_controlled_change` may include passive `context_governance` metadata with
estimated context units and a `start_projection_v1` digest. In `mode="observe"`
this is response telemetry only; it does not replace `status`, `edit_allowed`,
scope, or blast-radius safety fields. Default start responses now use a slim
blast summary when an immutable audit-backed artifact was stored. The summary
keeps `do_not_touch`, `do_not_touch_summary`, status, scope, workspace blocking
facts, and artifact identity inline. Full omitted blast evidence is fetched
read-only with `get_blast_artifact(root, run_id, blast_artifact_id)`; it is exact
historical evidence, not a fresh `get_blast_radius` recomputation.
