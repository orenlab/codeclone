...
| `get_relevant_memory`    | `root`, `scope`, `intent_id`, `symbols`, `max_records`, `include_stale`, `include_drafts`                                    | Ranked engineering memory for declared edit scope. Requires prior `codeclone memory init`. See [Engineering Memory](26-engineering-memory.md) |
...
| `manage_engineering_memory` | `root`, `action`, `record_type`, `statement`, `subject_path`, `text`, `intent_id`, `run_id` | Agent-side memory governance: `record_candidate`, `validate_claims`, `propose_from_receipt`. Human approve/reject/archive remain CLI-only (`codeclone memory approve\|reject\|archive`). See [Engineering Memory](26-engineering-memory.md) |
