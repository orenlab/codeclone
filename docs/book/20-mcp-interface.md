...
| `get_report_section`     | `run_id`, `section`, `family`, `path`, `offset`, `limit`                                                                                               | Return a canonical report section. Prefer targeted sections instead of `section="all"` unless the client truly needs the full canonical report. `metrics` is summary-only; `metrics_detail` is paginated/bounded, falls back to summary+hint when unfiltered, and can expose report-only families such as `god_modules` |
