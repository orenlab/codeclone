...
| 4    | `get_implementation_context` for scoped Python paths (optional but recommended) |
| 5    | Edit inside declared scope only                                                 |
| 6    | After-run when profile requires it                                              |
| 7    | `finish_controlled_change` with `changed_files` or `diff_ref`                   |
