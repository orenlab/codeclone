    ...
    "engineering_memory": MCPHelpTopicSpec(
        ...
        key_points=(
            ...
            (
                ...
                "`code spans` for paths/symbols/digests, ```lang fenced code "
                "blocks for multi-line snippets, **bold**/*italic*, "
                "lists nested at most once, compact tables, '> ' blockquotes "
                "for quoted rulings, bare URLs. A fence takes backticks and a "
                "language tag: only that body is shielded, so a ~~~ fence or "
                "an indented block refuses a snippet the backtick fence "
                "accepts. Security bans (typed rejects "
                "memory_md_image/memory_md_html/memory_md_link): images, raw "
                "HTML tags — <code> included, because one admitted tag makes "
                "the render surface parse author markup — and [text](url) "
                "links; put literal markup in a backtick code span or a fence "
                "instead. memory_md_heading_structure "
    ...
    "verification_profiles": MCPHelpTopicSpec(
        ...
        key_points=(
            ...
            (
                ...
                "manifest digest that no longer matches the bytes on disk proves "
                "the run did not read them, and is refused rather than treated "
                "as merely unproven. A matching stat is not evidence: a same-size "
                "rewrite keeps (mtime_ns, size) intact."
            ),
            (
                "Residual limitation: files analysis never reads, such as "
                "pyproject.toml, carry no manifest digest, so per-file observation "
