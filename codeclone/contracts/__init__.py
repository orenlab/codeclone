...
# numbers: they are the output of ``metrics.complexity_calibration``'s
# procedure over its pinned reference distribution, and
# ``tests/test_complexity_calibration.py`` reds if either constant stops
# equalling that output. The retired Y9-CFG values were 28 and 1 (from
# n=5194, 146 above 10, 5 above 20). The complexity BANDS are unchanged: 10
# and 20 were reviewed and kept, and the recalibration touched only these two
# permilles — never fail_health, never the bands, never the outlier term.
#
...
# Generated by metrics.complexity_calibration.reference_permilles(); pinned by
# tests/test_complexity_calibration.py. Do not hand-edit — recalibrate through
# the procedure with a fresh measurement and maintainer ratification.
