...
# differ from the ones that read well here (`where` for our predicate, `on` for
# our key, `new_data` for our records, `i` for a PyArrow column name). A
# positional-or-keyword declaration would claim those names are callable as
# keywords, which is false against the real objects. Results we discard are
# typed `object` rather than `None`: LanceDB returns a result record.
...
class _LanceTable(Protocol):
    # Read-only: LanceDB exposes `schema` as a property, and a bare annotation
    # would claim this module may also write it.
    @property
    ...
    def delete(self, clause: str, /) -> object: ...
