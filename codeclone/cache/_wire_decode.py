...
def _decode_wire_unit(value: object, filepath: str) -> UnitDict | None:
    # Load-bearing in concert with the cache trust envelope, not redundant
    ...
    # cache whose "v" != CACHE_VERSION and whose envelope fails
    # integrity.verify_cache_envelope_checksum -- a keyless checksum taken over
    # the versioned pre-image {v, payload} (cache_envelope_checksum), so a "v"
    # rewritten to match while the payload stays old-format is still rejected.
    # That CACHE_VERSION-bound gate and this tolerance are two halves of one
