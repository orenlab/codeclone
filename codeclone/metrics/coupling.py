...
# Builtin-name registry, pinned so coupling facts are interpreter-independent.
#
# ``dir(builtins)`` made the exclusion filter a property of the *running*
# interpreter: a repository referencing a name that is a builtin only on newer
# CPython (``frozendict``, ``sentinel``, ... on 3.15) counted the edge on one
# interpreter and dropped it on another. The registry below is the union of
# ``dir(builtins)`` across the supported CPython range 3.10-3.15; union rather
# than per-tag because the set feeds an exclusion filter, so covering every
# supported interpreter means a name that denotes a builtin anywhere is never
# reported as a collaborator edge.
#
# Bump discipline: extend this set (never shrink it) when a new CPython minor
# joins the support matrix, and review DESIGN_METRICS_ALGORITHM_REVISION when
# an addition can move published coupling values. The tripwire test
# ``test_builtin_name_registry_is_interpreter_pinned`` fails when the running
# interpreter's builtins outgrow the registry.
