...
| Engineering Memory (`codeclone/memory/*`, `codeclone/config/memory*.py`, memory CLI/MCP tools)                                      | `docs/mcp.md`, `docs/book/20-mcp-interface.md`, `CLAUDE.md`, plugin skills, `CHANGELOG.md`                                                                        | `tests/test_memory_*.py`, MCP memory tool tests in `tests/test_mcp_service.py`, contract snapshots when tool schemas change                                                                                                                                                                                                           | schema version bumps, store contract semantics, governance transitions (18.4+)                                            | `ENGINEERING_MEMORY_SCHEMA_VERSION`, MCP memory tool payloads, CLI memory commands, or SQLite DDL change   |
