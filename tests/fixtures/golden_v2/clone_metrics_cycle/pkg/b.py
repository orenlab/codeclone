...
class ServiceB:
    def __init__(self) -> None:
        self.peer: ServiceA | None = None

    def compute(self, values: list[int]) -> tuple[list[int], int]:
        return transform_shared_b(values)


def transform_shared_b(items: list[int]) -> tuple[list[int], int]:
    if not items:
        return [], 0
